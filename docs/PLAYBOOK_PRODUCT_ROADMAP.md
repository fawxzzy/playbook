...
Recent implementation note: the reusable workflow-pack boundary is now frozen in `docs/contracts/WORKFLOW_PACK_REUSE_CONTRACT.md` and published through `pnpm playbook contracts --json`, bundling local verification truth, workflow promotion truth, continuity-doctrine inheritance, versioning policy, and consumer rules into one downstream-discoverable owner surface while also routing consumer continuity/handoff adoption back through the registry-published `docs/contracts/PLAYBOOK-CONTRACT.md`, now machine-tagged as `core_continuity_doctrine` and exposed through a direct `artifacts.contractRoles` role-to-path lookup plus paired `exportPath` so downstream tooling can discover both the owner doctrine document and its canonical machine export semantically instead of by path convention alone. That semantic role now also ships directly inside the canonical contract export as `continuity_requirements.contract_role`, carries through machine-consumed convergence source-inventory and repo-scorecard reports as additive `contractRole` / `contractRoles` metadata plus paired `contractExportPath` / `contractExportPaths`, and is preserved and verified by the published input artifacts plus validators end to end instead of appearing only during report projection. The external bootstrap proof surface now carries the same doctrine pairing directly under `status proof` JSON (`continuity.doctrine.role`, `path`, `export_path`, `registration_state`), the adjacent continuity read surfaces `query runs` and `session show` now do the same, and the startup bootstrap surfaces `ai-context`, `ai-contract`, and `context` now also preserve that same `continuity.doctrine` pairing directly, so startup, run-inspection, and session-inspection consumers can recover continuity lineage plus doctrine identity from one read-first operator surface family.
Recent implementation note: the adjacent capture, retrieval, and promotion seams `pnpm playbook memory --json`, `pnpm playbook knowledge --json`, and `pnpm playbook promote --json` now also preserve the same additive `continuity.doctrine` pairing on both success and deterministic failure envelopes, so automation can carry doctrine identity through memory, knowledge, and promotion workflows without a second registry/bootstrap lookup.
...
Recent implementation note: Codex-ready mutating prompt surfaces now carry explicit acceptance-criteria and diff-proof sections across `route`, `orchestrate`, worker lane rendering, and fleet-adoption execution packaging so governed completion does not rely on summary text or implicit path assumptions.
