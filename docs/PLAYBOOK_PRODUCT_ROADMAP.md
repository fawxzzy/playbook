...
Recent implementation note: `PB-V10-LIFELINE-OBSERVER-HOME-001` pins Playbook's Lifeline-only start surface to the canonical Atlas runtime home with a portable global target plus explicit Observer root, keeping both graceful-stop lifecycle telemetry and Observer state out of the source checkout while direct Observer CLI invocations retain their existing cwd and caller-supplied root behavior.
