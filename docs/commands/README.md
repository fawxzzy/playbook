...
- Mutating Codex prompt surfaces emitted by `route`, `orchestrate`, `workers`, and fleet-adoption execution packaging now require explicit `Acceptance Criteria`, `Expected Changed Paths`, `Expected Unchanged Paths`, and `Blocked / Skipped Reporting Rules`; summary text is not proof for governed completion.
...
- `docs/contracts/WORKFLOW_PACK_REUSE_CONTRACT.md` freezes the reusable workflow-pack bundle for downstream adoption. Consumers should discover it through `pnpm playbook contracts --json`, reuse the canonical local verification, workflow promotion, versioning, and consumer-boundary contracts together, and resolve the continuity doctrine through the registry role lookup (`artifacts.contractRoles` -> `core_continuity_doctrine`) instead of depending on path recall alone.
...
- Bootstrap continuity note: `ai-context`, `ai-contract`, and `context` now each expose additive `continuity.doctrine` metadata so bootstrap consumers can recover the canonical continuity doctrine role, owner contract path, canonical export path, and fail-closed registration state directly from the recommended startup command family.
- Ask repo-context continuity note: `pnpm playbook ask ... --repo-context` now carries the same continuity doctrine pairing inside its trusted prompt-context bundle and source list, so agent-facing startup hydration no longer has to infer the owner doctrine from the AI contract alone.
...
- `status proof` is the canonical external-consumer bootstrap proof surface for proving runtime + CLI + docs/artifact + execution/governance readiness in one read-only flow. It now also reads existing parallel-work artifacts (`lane-state`, `worker-results`, `docs-consolidation-plan`, and guarded-apply outcomes) to emit one compact operator brief while keeping required automation truth in the canonical `proof` payload and deterministic additive detail in JSON, including failure-domain ownership fields (`failureDomains`, `primaryFailureDomain`, `domainBlockers`, `domainNextActions`) mapped to canonical domains (`contract_validation`, `runtime_execution`, `ci_bootstrap`, `sync_drift`, `governance_planning`), additive continuity/evidence summary fields under `continuity`, additive continuity-doctrine bootstrap fields under `continuity.doctrine` (`role`, registered owner `path`, canonical `export_path`, registration state), and additive longitudinal read-model fields under `longitudinal_state` (unresolved risk summary, recurring cluster summary, verification lineage, lifecycle rollup).
...
- `session show` now includes compact continuity/evidence operator signals (active refs, pinned evidence refs, latest run/receipt lineage, stale/missing continuity markers), additive JSON fields under `continuity`, additive continuity-doctrine bootstrap fields under `continuity.doctrine` (`role`, registered owner `path`, canonical `export_path`, registration state), and additive `longitudinal_state` JSON + one-line text brief.
...
`query runs` now emits additive continuity evidence in JSON (`continuity.doctrine`, `continuity.session`, `continuity.lineage`, `continuity.staleSignals`) plus additive `longitudinal_state` JSON, and keeps text output brief with one-line doctrine, continuity, and longitudinal summaries.
`memory --json`, `knowledge --json`, and `promote --json` now also emit additive `continuity.doctrine` metadata on both success and deterministic failure envelopes, so capture, retrieval, and promotion seams preserve the same doctrine pairing as the existing bootstrap and run/session read surfaces.
