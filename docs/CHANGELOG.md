...
- WHAT: Updated `pnpm start:lifeline` to select the portable global target `../../runtime/playbook/observer` and pass the explicit Observer root `.`, added deterministic topology plus graceful-stop regression proof, and documented the Lifeline-only runtime-home boundary. WHY: A supervisor-owned lifecycle must keep both Observer state and Playbook runtime telemetry out of the source checkout, including when `endRuntimeCycle` runs after a normal graceful stop.
- Rule: Runtime homes are explicit; a supervisor-owned start surface must pass the canonical runtime home explicitly.
- Pattern: Stable repository working directory + portable relative global target + explicit Observer-local root -> deterministic registry and lifecycle telemetry placement outside the source checkout.
