...
const buildContinuitySnapshot = (cwd, runs) => {
    ...
    const latestReceiptRefs = latestRun
        ? Array.from(new Set(Object.values(latestRun.lanes).flatMap((lane) => lane.receipt_refs)))
...
const printText = (field, result, architectureRoleInference) => {
    ...
    if (Array.isArray(result)) {
        ...
        if (typeof firstValue === 'object' && firstValue !== null && 'name' in firstValue && 'dependencies' in firstValue) {
            for (const moduleEntry of result) {
...
const printDependenciesText = (payload) => {
    console.log('Dependencies');
    console.log('────────────');
    if (payload.module) {
        const values = payload.dependencies;
...
const printImpactText = (payload) => {
    ...
    console.log(`Direct dependents: ${payload.impact.directDependents.length > 0 ? payload.impact.directDependents.join(', ') : 'none'}`);
    console.log(`Transitive dependents: ${payload.impact.dependents.length > 0 ? payload.impact.dependents.join(', ') : 'none'}`);
    console.log(`Risk: ${payload.impact.risk.level} (${payload.impact.risk.score.toFixed(2)})`);
};
const printDocsCoverageText = (payload) => {
    console.log('Documentation Coverage');
    console.log('──────────────────────');
    console.log('');
    console.log('Documented modules');
    const documented = payload.modules.filter((entry) => entry.documented);
    ...
    const undocumented = payload.modules.filter((entry) => !entry.documented);
...
const printRuleOwnersText = (payload) => {
    if ('rule' in payload) {
...
const printModuleOwnersText = (payload) => {
...
const printTestHotspotsText = (payload) => {
...
export const runQuery = async (cwd, commandArgs, options) => {
            ...
            if (!options.quiet) {
                ...
                if (payload.candidates.length === 0) {
                    ...
                    for (const candidate of payload.candidates) {
                        ...
                        console.log(`${candidate.id} score=${candidate.promotionScore} confidence=${candidate.confidence} weighted=${convergence.weightedScore} priority=${convergence.suggestedPriority} cluster=${cluster}`);
                    }
                }
            }
            return ExitCode.Success;
    ...
    try {
        ...
        const result = {
            ...
            memorySources: query.memorySources,
            knowledgeHits: query.knowledgeHits,
            recentRelevantEvents: query.recentRelevantEvents,
            memoryKnowledge: query.memoryKnowledge
        };
        if (options.format === 'json') {
