...
The app process started by Lifeline is `pnpm start:lifeline`. That dedicated surface keeps the loopback host and port fixed at `127.0.0.1:4300`, selects `../../runtime/playbook/observer` through the existing global `--repo` option, and passes the Observer-local `--root .` explicitly. In the canonical Atlas topology, where this checkout is `repos/playbook`, both the Playbook lifecycle telemetry root and Observer home resolve to `runtime/playbook/observer`, so normal start and graceful stop do not create mutable `.playbook` state in the source checkout. Normal `pnpm playbook observer ...` commands retain their existing cwd and explicit-`--root` behavior.
