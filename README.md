...
Recent implementation note: `pnpm playbook knowledge atlas-admit --artifact <path> --atlas-contracts-root <dir> --json` now consumes Atlas-owned `atlas.knowledge-candidate.v2` artifacts through the official `@atlas/contracts` `./validator` export, preserves exact candidate identity, source artifact path and byte hash, classified provenance, and an explicit candidate-review-only `accept` disposition, and writes only the deterministic governed review queue at `.playbook/memory/atlas-knowledge-candidates.json` with a correlated consumer receipt.
...
Pattern: Candidate-only intake preserves exact identity, source bytes, provenance, and owner disposition and emits a deterministic correlated receipt.
...
The app process started by Lifeline is `pnpm start:lifeline`. That dedicated surface creates the relative `../../runtime/playbook/observer` directory when absent, keeps the loopback host and port fixed at `127.0.0.1:4300`, selects that directory through the existing global `--repo` option, and passes the Observer-local `--root .` explicitly. In the canonical Atlas topology, where this checkout is `repos/playbook`, both the Playbook lifecycle telemetry root and Observer home resolve to `runtime/playbook/observer`, so first start, normal start, and graceful stop do not create mutable `.playbook` state in the source checkout. Normal `pnpm playbook observer ...` commands retain their existing cwd and explicit-`--root` behavior.
...
`pnpm playbook knowledge` provides read-only inspection for normalized evidence, candidate knowledge, promoted doctrine, and superseded knowledge. Its explicit `atlas-admit` subcommand is the narrow exception: it writes only the governed Atlas review-candidate queue and never promotes or mutates doctrine. Each admitted record preserves the portable Atlas artifact path, exact source-byte SHA-256, and explicit `accept` disposition scoped only to candidate review.
